#!/usr/bin/env python3
"""Approximate filled SVG stroke outlines as centerline stroke paths.

This tool is intentionally self-contained so it can run through uv without
installing image-processing packages. It rasterizes a filled path, thins the
filled mask to a one-pixel skeleton, traces the longest skeleton chain by
default, and serializes the chain as a stroked SVG path.
"""
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert a filled SVG outline to an approximate centerline stroke path."
    )
    input_group = parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument("--path", help="Raw SVG path d data.")
    input_group.add_argument("--input", "-i", help="SVG file or text file containing path data.")
    parser.add_argument("--output", "-o", help="Write output to this file instead of stdout.")
    parser.add_argument(
        "--emit",
        choices=("path", "svg", "d"),
        default="path",
        help="Output a path element, a complete SVG, or only path d data.",
    )
    parser.add_argument(
        "--mode",
        choices=("longest", "all"),
        default="longest",
        help="Trace only the longest skeleton chain or all chains above --min-length.",
    )
    parser.add_argument(
        "--scale",
        type=float,
        default=2.0,
        help="Raster pixels per SVG unit before --max-size limiting.",
    )
    parser.add_argument(
        "--max-size",
        type=int,
        default=1600,
        help="Maximum raster width or height. Set 0 to disable limiting.",
    )
    parser.add_argument(
        "--curve-samples",
        type=int,
        default=24,
        help="Base samples per cubic/quadratic segment while flattening.",
    )
    parser.add_argument(
        "--simplify",
        type=float,
        default=6.0,
        help="Douglas-Peucker centerline simplification tolerance in SVG units.",
    )
    parser.add_argument(
        "--min-length",
        type=float,
        default=20.0,
        help="Drop traced skeleton chains shorter than this SVG-unit length.",
    )
    parser.add_argument(
        "--stroke-width",
        default="auto",
        help="Stroke width in SVG units, or 'auto' to estimate from the outline.",
    )
    parser.add_argument(
        "--linecap",
        default="round",
        help="stroke-linecap value for path/svg output.",
    )
    parser.add_argument(
        "--linejoin",
        default="round",
        help="stroke-linejoin value for path/svg output.",
    )
    parser.add_argument(
        "--decimals",
        type=int,
        default=3,
        help="Decimal places for serialized coordinates and stroke width.",
    )
    parser.add_argument(
        "--polyline",
        action="store_true",
        help="Serialize centerlines with L commands instead of Catmull-Rom cubic smoothing.",
    )
    parser.add_argument(
        "--fill-rule",
        choices=("evenodd",),
        default="evenodd",
        help="Fill rule used by the rasterizer. Currently only evenodd is supported.",
    )
    parser.add_argument(
        "--svg-paths",
        choices=("first", "all"),
        default="first",
        help="When --input is an SVG, convert only the first path or every path into stroked centerlines.",
    )
    parser.add_argument(
        "--keep-failed",
        action="store_true",
        help="For --svg-paths all, keep original path elements that fail centerline conversion.",
    )
    return parser.parse_args(argv)


def fail(message: str) -> None:
    print(f"svg-centerline-cli: {message}", file=sys.stderr)
    sys.exit(1)


def read_path_data(args: argparse.Namespace) -> str:
    if args.path:
...
def as_float(token: str) -> float:
    try:
        ...
        fail(f"Invalid numeric token: {token}")
...
def quad_point(p0: Point, p1: Point, p2: Point, t: float) -> Point:
    mt = 1.0 - t
    return Point(
        mt**2 * p0.x + 2 * mt * t * p1.x + t**2 * p2.x,
        mt**2 * p0.y + 2 * mt * t * p1.y + t**2 * p2.y,
    )
...
def flatten_path(path_data: str, curve_samples: int) -> list[list[Point]]:
    ...
    def read_number(label: str) -> float:
        ...
        if i >= len(tokens) or is_command(tokens[i]):
            fail(f"Arc command requires {label}")
    ...
    def read_arc_flag(label: str) -> int:
        ...
        if token[0] in ("0", "1"):
            ...
            if not tokens[i]:
                i += 1
            return flag
        fail(f"Arc command {label} must be 0 or 1")
    ...
    if subpath:
        subpaths.append(subpath)
    if not subpaths:
        fail("Path did not produce any drawable subpaths")
    return subpaths
...
def to_pixel(point: Point, ctx: RasterContext) -> tuple[float, float]:
    return (
        (point.x - ctx.min_x) * ctx.scale + ctx.pad,
        (point.y - ctx.min_y) * ctx.scale + ctx.pad,
    )
...
def to_svg(pixel: tuple[int, int], ctx: RasterContext) -> Point:
    row, col = pixel
    return Point(
        (col + 0.5 - ctx.pad) / ctx.scale + ctx.min_x,
        (row + 0.5 - ctx.pad) / ctx.scale + ctx.min_y,
    )
    ...
    for row in range(ctx.height):
        ...
        for left, right in zip(intersections[0::2], intersections[1::2]):
            ...
            for col in range(start, end + 1):
                filled.add((row, col))
    if not filled:
        fail("Rasterization produced an empty mask")
    return filled
    ...
    while changed:
        ...
        for step in (0, 1):
            ...
            if remove:
                ...
                changed = True
    if not foreground:
        fail("Skeletonization produced an empty centerline")
    return foreground
...
def estimate_stroke_width(
    ...
    if stroke_width != "auto":
        try:
            ...
            fail("--stroke-width must be a number or 'auto'")
        if width <= 0:
            fail("--stroke-width must be greater than zero")
        return width

    values = sorted(distances[pixel] / scale for pixel in skeleton if distances.get(pixel, 0) > 0)
    if not values:
        fail("Could not estimate stroke width from skeleton distances")
...
def centerline_path_data(path_data: str, args: argparse.Namespace) -> tuple[str, float, RasterContext]:
    subpaths = flatten_path(path_data, args.curve_samples)
    ctx = make_context(subpaths, args.scale, args.max_size)
    ...
    stroke_width = estimate_stroke_width(args.stroke_width, skeleton, distances, ctx.scale)

    min_length_px = max(0.0, args.min_length * ctx.scale)
    pixel_paths = trace_skeleton(skeleton, args.mode, min_length_px)
    svg_paths: list[str] = []
    for pixel_path in pixel_paths:
        points = [to_svg(pixel, ctx) for pixel in pixel_path]
        points = simplify_points(points, max(0.0, args.simplify))
        if len(points) < 2:
            continue
        svg_paths.append(serialize_polyline(points, args.decimals) if args.polyline else serialize_smooth(points, args.decimals))

    if not svg_paths:
        fail("No centerline paths survived simplification")
...
def centerline_svg_text(svg_text: str, args: argparse.Namespace) -> str:
    try:
        root = ET.fromstring(svg_text)
    except ET.ParseError as error:
        fail(f"Could not parse SVG input: {error}")
    ...
    def walk(element: ET.Element, inherited: dict[str, str]) -> None:
        ...
        if local_name(element.tag) == "path" and element.attrib.get("d"):
            d = element.attrib["d"]
            try:
                center_d, stroke_width, _ctx = centerline_path_data(d, args)
            except SystemExit:
                if not args.keep_failed:
                    raise
                output_paths.append(
                    f'<path d="{escape_attr(d)}" fill="{escape_attr(current.get("fill", "black"))}"/>'
                )
                return
            ...
            style = (
                "fill: none; "
                f"stroke: {color}; "
                f"stroke-linecap: {args.linecap}; "
                f"stroke-width: {fmt(stroke_width, args.decimals)}px; "
                f"stroke-linejoin: {args.linejoin};"
            )
    ...
    walk(root, {})
    if not output_paths:
        fail("No path elements found in SVG input")
...
def build_output(
    ...
    style = (
        ...
        f"stroke-width: {fmt(stroke_width, decimals)}px; "
    ...
    return (
        ...
        f'viewBox="{fmt(min_x, decimals)} {fmt(min_y, decimals)} {fmt(width, decimals)} {fmt(height, decimals)}">'
        f"{path}</svg>"
    )
...
def main(argv: list[str]) -> int:
    ...
    if args.input and args.svg_paths == "all":
        ...
        if "<svg" not in text[:500].lower():
            fail("--svg-paths all requires an SVG input file")
        output = centerline_svg_text(text, args)
    else:
        path_data = read_path_data(args)
        d, stroke_width, ctx = centerline_path_data(path_data, args)
        output = build_output(d, args.emit, stroke_width, args.linecap, args.linejoin, args.decimals, ctx)
    if args.output:
